from flask import Flask,Blueprint,request,render_template,make_response,redirect,url_for,session
...
def set_email(): #라우팅 경로와 똑같이 함수명을 해주어야 헷갈리지 않음
    ...
    if request.method=="GET": 
        ...
        return redirect(url_for('blog.blog_fullstack1')) #url_for을 써주면 blueprint,해당 함수명를 좀 더 직관적으로 확인할 수 있다는 장점
    ...
    #post인 경우
        ...
        #print('set_email',request.form['user_email']) #hello@khu.ac.kr
        #print('set_email',request.form['blog_id']) #hello@khu.ac.kr
        user=User.create(request.form['user_email'],request.form['blog_id']) #사용자 등록
...
def blog_fullstack1():
    ...
    if current_user.is_authenticated:
        ...
        BlogSession.save_session_info(session['client_id'],'anonoymous',webpage_name) #로그인 안 된 상태라 이메일을 아직 모름
...
#main server에서 blog_abtest가 이미 blueprint로 등록이 되어 있기 때문에
#app에다가 before_request를 하더라도 blue print(blog_abtest)로 연동되어있으므로 
#각 라우팅 전에 before_request가 호출된다.